...
class MatchController extends Controller
{
            ...
            function ($row)
            {
                ...
                if($row->getField('date') <= $today){       
                    ...
                    if ($row->getField('bp') > $row->getField('bc')) { 
                        $row->setColor('#00ff00');  // match gagné (vert)
                    }elseif ($row->getField('bp') < $row->getField('bc')) {
                        $row->setColor('#FF0000');  // match perdu (rouge)
                    }else{
                        $row->setColor('#FFFF00');  // match nul (jaune)
                    }             
                }
                return $row;
            }
    ...
    public function showAction($id)
    {
        ...
        // on vérifie que le demandeur appartient au club sinon on le jarte!
