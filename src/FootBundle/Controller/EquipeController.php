...
class EquipeController extends Controller
{
    ...
    public function showAction($id)
    {
        ...
        // calcul des stats du championnat
        $typeMatch = $em->getRepository('FootBundle:Typematch')->findByType('CHAMPIONNAT');
        // Matches joués en championnat
        $Matches = $em->getRepository('FootBundle:Match')->getPlaiedMatch($entity,$typeMatch);
        // nb total de matches du championnat
        $nbTotalMatches = $em->getRepository('FootBundle:Match')->nbTotal($entity,$typeMatch);
        // nb points à prendre
        $nbMaxPoints = count($Matches) * $entity->getScoring()->getG();
        // nb points gagnés
        $nbWonPoints = 0;
        $g = $entity->getScoring()->getG();
        $n = $entity->getScoring()->getN();
        $p = $entity->getScoring()->getP();
        foreach ($Matches as $key => $match) {
            if($match->getBp() > $match->getBc()){
                $nbWonPoints += $g;
            }elseif($match->getBp() < $match->getBc()){
                $nbWonPoints += $p;
            }else{
                $nbWonPoints += $n;
            } 
        ...
        // taux de réussite
        $reussiteTx = ($nbWonPoints / $nbMaxPoints) * 100;
        $statistiques = array(  'nbTotalMatches'        => $nbTotalMatches,
                                'nbPlaiedMatches'       => count($Matches),
                                'nbMaxPoints'           => $nbMaxPoints,
                                'nbWonPoints'           => $nbWonPoints,
                                'reussiteTx'            => $reussiteTx
                            );
        
        
        // Liste des Matchs
        $source = new Entity('FootBundle:Match');
        // Ne selectionner que les matches relatifs à cette équipe
        $source->manipulateQuery(
            function ($query) use ($id, $typeMatch)
            {
                $query	->andWhere('_equipe.id = '.$id)
                        ->andWhere('_typematch.id = '.$typeMatch[0]->getId());
        ...
        // Réglage des filtres
        if(!$this->get('security.context')->isGranted('ROLE_SUPER_ADMIN_SLAD')){
            $grid->getColumn('equipe.nom')->setVisible(false);
            $grid->getColumn('typematch.type')->setVisible(false);
        }
        $source->manipulateRow(
            function ($row)
            {
        ...
        // render
        return $grid->getGridResponse('::Appli\Equipe\show.html.twig',
                                        array(  'grid'          =>  $grid,
                                                'entity'        =>  $source,
                                                'equipe'        =>  $entity,
